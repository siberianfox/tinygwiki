    ...
    Revised by: Alden - 10/29/12 - 6:36 PM EST - took a crack at revised Ack and Qr packet formats`
...
Most commands sent to TinyG fall into one of two broad types:
	...
	Config  | "$" cmds, e.g: $xvm=10000, {"xvm":10000}  | Config commands set the machine configuration (e.g. xvm=10000). Config commands are not accepted during a cycle and will return an error if attempted in-cycle.
