...
Xboot has been set up as an AVRStudio4 project (Windows only). use the xboot.aps project file. This project uses the Studio4 auto-generated makefile in the `default` directory instead of the native xboot Makefile in the working directory. The following needs to be done for this to work.

1. Config.h is not generated by the project. You must generate config.h by running `make conf/x192a3.conf.mk`. If the conf file changes you must run this again to refresh config.h
2. Setup the xboot project configuration in `Project \ Configuration Options`
<pre>
in General:
frequency = 32000000   (32 Mhz)
optimization = -Os
do not use external makefile (unchecked)
